...
export class Slideshow {
  ...
  gap = getComputedStyle(document.documentElement).getPropertyValue(
    "--slide-gap"
  );
  ...
  constructor(DOM_el) {
    ...
    this.DOM.contentItems.forEach((item) =>
      this.contentItems.push(new ContentItem(item))
    );
  ...
  open(stackItem) {
    ...
    this.contentItems[this.current].DOM.el.classList.add(
    ...
    Flip.from(state, {
      ...
      .to(
        this.contentItems[this.current].DOM.texts,
        {
          duration: 1,
          ease: "expo",
      ...
      .to(
        [this.DOM.navArrows.prev, this.DOM.navArrows.next],
        {
          duration: 1,
          ease: "expo",
          startAt: {
          ...
          opacity: (pos) =>
            (this.current === 0 && !pos) ||
            (this.current === this.totalItems - 1 && pos)
              ? 0
              : 1,
        },
        0
  ...
  navigate(direction) {
    ...
    gsap.set(this.DOM.navArrows.next, {
    ...
    gsap
      ...
      .to(this.DOM.el, {
        ...
        y:
          direction === "next"
            ? `-=${winsize.height / 2 + winsize.height * 0.02}`
            : `+=${winsize.height / 2 + winsize.height * 0.02}`,
        onComplete: () => {
          this.isAnimating = false;
        },
      ...
      .to(
        this.contentItems[previousCurrent].DOM.texts,
        {
          duration: 0.2,
          ease: "power1",
          yPercent: direction === "next" ? 101 : -101,
          onComplete: () =>
            this.contentItems[previousCurrent].DOM.el.classList.remove(
              "content__item--current"
            ),
        },
        0
